...
public class RoxyFilemanHandler : IHttpHandler, System.Web.SessionState.IRequiresSessionState
{
    ...
    string confFile = "/Areas/Admin/scripts/RoxyFileManager/conf.json";
    ...
    private string GetLangFile(){
        string filename = "/Areas/Admin/scripts/RoxyFileManager/lang/" + GetSetting("LANG") + ".json";
        if (!File.Exists(_context.Server.MapPath(filename)))
            filename = "/Areas/Admin/scripts/RoxyFileManager/lang/en.json";
        return filename;
    }
    ...
    protected string GetFilesRoot(){
        ...
        if (ret == "")
            ret = "F:/Uploads/Web";
        //_context.Server.MapPath("~/Uploads/Web");
        else
            ret = FixPath(ret);
        return ret;
    }
    ...
    protected void CheckPath(string path)
    {
        ...
        {
            throw new Exception("Access to " + path + " is denied" + HttpContext.Current.User.Identity.Name);
        }
    }
