...
namespace ParvazPardaz.Web.AutoMapperProfile
{
    public class ReviewProfile : Profile
    {
        ...
        {
            CreateMap<Review, AddReviewViewModel>().IgnoreAllNonExisting();
            CreateMap<AddReviewViewModel, Review>().IgnoreAllNonExisting();

            CreateMap<EditReviewViewModel, Review>().IgnoreAllNonExisting();
            CreateMap<Review, EditReviewViewModel>().IgnoreAllNonExisting();

            CreateMap<Review, GridReviewViewModel>().ForMember(x => x.CreatorDateTime, m => m.MapFrom(model => (model.CreatorDateTime.HasValue) ? model.CreatorDateTime : null))
