(function ($) {
	...
	$.fn.shareButtons = function (url, options) {
		...
		var shareURLs = {
			...
			'telegram': 'http://www.telegram.me/share/url?url=' + url_encoded,
		...
		if (options.telegram) {
		    ...
		    if (options.telegram.name) {
		        shareURLs['telegram'] += '&name=' + encodeURIComponent(options.telegram.name);
		    }

		    if (options.telegram.description) {
		        shareURLs['telegram'] += '&description=' + encodeURIComponent(options.telegram.description);
		    }
		...
		this.each(function(i){
			...
			if (options.telegram) {
			    socialButtonsPopUp.append($('<a class="fa fa-telegram" href="' + shareURLs.telegram + '" ></a>'));
			}
		});
