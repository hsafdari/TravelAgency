        ...
        $('.price-slider').each(function() {
            ...
            priceslider.slider({
                ...
                slide: function (event, ui) {
                    ...
                    var $this=$(this),
                        values=ui.values;
                    priceslider.siblings('.price_slider_amount').find('.from').text('تومان' + values[0]);
                    priceslider.siblings('.price_slider_amount').find('.to').text('تومان' + values[1]);

                    var ele = $(".price_slider_amount .price_label span");
                    var fromPrice = parseFloat($(ele[0]).html().split('تومان')[1]);
                    var toPrice = parseFloat($(ele[1]).html().split('تومان')[1])
                    var eleFlights = $(".filter-item-wrapper");
                    for (var i = 0; i < eleFlights.length; i++) {
                        var price = parseFloat($(eleFlights[i]).attr('data-price'));
                        if (price < fromPrice || price > toPrice) {
                            $(eleFlights[i]).hide('blind');
                        }
                        else {
                            $(eleFlights[i]).show('blind');
                        }
                    }
                }
            });
