var Index = function () {
    ...
    var runChart1 = function () {
        ...
        var plot = Jq.plot(Jq("#placeholder-h1"), [{
            data: pageviews,
            label: "Unique Visits"
        }, {
            data: visitors,
        ...
        Jq("#placeholder-h1").bind("plothover", function (event, pos, item) {
    ...
    var runChart2 = function () {
        ...
        Jq.plot('#placeholder-h2', data_pie, {
            series: {
                pie: {
                    show: true,
                    radius: 1,
    ...
    var runChart3 = function () {
        ...
        var plot = Jq.plot("#placeholder-h3", [getRandomData()], {
            grid: {
                borderWidth: 1,
                borderColor: '#eeeeee'
            },
    ...
    var runSparkline = function () {
        Jq(".sparkline_line_good span").sparkline("html", {
            type: "line",
            fillColor: "#B1FFA9",
            lineColor: "#459D1C",
            width: "50",
        ...
        Jq(".sparkline_line_bad span").sparkline("html", {
            type: "line",
            fillColor: "#FFC4C7",
            lineColor: "#BA1E20",
            width: "50",
        ...
        Jq(".sparkline_line_neutral span").sparkline("html", {
            type: "line",
            fillColor: "#CCCCCC",
            lineColor: "#757575",
            width: "50",
        ...
        Jq(".sparkline_bar_good span").sparkline('html', {
            type: "bar",
            barColor: "#459D1C",
            barWidth: "5",
            height: "24"
        });
        Jq(".sparkline_bar_bad span").sparkline('html', {
            type: "bar",
            barColor: "#BA1E20",
            barWidth: "5",
            height: "24"
        });
        Jq(".sparkline_bar_neutral span").sparkline('html', {
            type: "bar",
            barColor: "#757575",
            barWidth: "5",
            height: "24"
    ...
    var runEasyPieChart = function () {
        ...
        Jq('.easy-pie-chart .bounce').easyPieChart({
            animate: 1000,
            size: 70
        });
        Jq('.easy-pie-chart .cpu').easyPieChart({
            animate: 1000,
            lineWidth: 3,
            barColor: '#35aa47',
            size: 70
    ...
    var runFullCalendar = function () {
        ...
        /* initialize the calendar
		 -----------------------------------------------------------------*/
        var Jqmodal = Jq('#event-management');
        Jq('#event-categories div.event-category').each(function () {
        ...
        var calendar = Jq('#calendar').fullCalendar({
            ...
            drop: function (date, allDay) { // this function is called when something is dropped
                ...
                var originalEventObject = Jq(this).data('eventObject');
                var JqcategoryClass = Jq(this).attr('data-class');
                // we need to copy it, so that multiple events don't have a reference to the same object
                var copiedEventObject = Jq.extend({}, originalEventObject);
                // assign it the date that was reported
                copiedEventObject.start = date;
                copiedEventObject.allDay = allDay;
                if (JqcategoryClass)
                    copiedEventObject['className'] = [JqcategoryClass];
                // render the event on the calendar
                // the last `true` argument determines if the event "sticks" (http://arshaw.com/fullcalendar/docs/event_rendering/renderEvent/)
                Jq('#calendar').fullCalendar('renderEvent', copiedEventObject, true);
            ...
            select: function (start, end, allDay) {
                ...
                Jqmodal.find('.remove-event').hide().end().find('.save-event').show().end().find('.modal-body').empty().prepend(form).end().find('.save-event').unbind('click').click(function () {
                    form.submit();
                });
                Jqmodal.find('form').on('submit', function () {
                    title = form.find("input[name='title']").val();
                    JqcategoryClass = form.find("select[name='category'] option:checked").val();
                    if (title !== null) {
            ...
            eventClick: function (calEvent, jsEvent, view) {
                ...
                Jqmodal.find('.remove-event').show().end().find('.save-event').hide().end().find('.modal-body').empty().prepend(form).end().find('.remove-event').unbind('click').click(function () {
